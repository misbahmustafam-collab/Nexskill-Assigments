...
df['Date'] = pd.to_datetime(df['Item_Visibility'])

# Create numeric features
df['year'] = pd.to_datetime(df['Item_Visibility']).dt.year
df['month'] = pd.to_datetime(df['Item_Visibility']).dt.month
df['day'] = pd.to_datetime(df['Item_Visibility']).dt.day
...
print(df['Outlet_Identifier'].values) # [2.5 5.1 3.2 8.5 3.5 1.5 9.2 ... ]
print(df['Outlet_Identifier'].values.shape) # (25,)
...
# Convert Date string to datetime
df['Item_Visibility'] = pd.to_datetime(df['Item_Visibility'])

# Create numeric features
df['year'] = df['Item_Visibility'].dt.year
df['month'] = df['Item_Visibility'].dt.month
df['day'] = df['Item_Visibility'].dt.day
...
variables = ['Item_Weight', 'Item_MRP']

for var in variables:
    sns.regplot(x=var, y='Item_MRP', data=df)
    plt.title(f'Regression Plot of {var} and Item_MRP')
    plt.show()
